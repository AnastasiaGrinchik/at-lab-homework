...
export class FullShoppingCartOverview extends AwesomeShopHomePage {
    ...
    async addCoupon(couponText) {
        ...
        let buttonApllyCoupon = await contentCart.$(
            this.buttonApplyCouponXpath
        );
    ...
    async getInformationAboutCoupon() {
        ...
        let sumOfDiscountWithCoupon = await contentCart.$(
            this.couponDiscountXpath
        );
    ...
    async checkoutOrder() {
        ...
        await contentCart.waitForClickable(
            elementToBeClickable(buttonCheckout)
        );
